...
const adminPagePath = new URL("../../src/pages/admin/index.astro", import.meta.url);
...
test("admin clears the old saved list view once so the new default sort takes effect", async () => {
	...
	assert.match(adminPage, /var SORT_PREFERENCE_MIGRATION_KEY = "qingge-admin-sort-v1";/);
...
test("every post references an existing local cover image", async () => {
	const postFiles = (await readdir(postsPath)).filter((file) => file.endsWith(".md"));

	for (const file of postFiles) {
		const source = await readFile(new URL(`../../src/content/posts/${file}`, import.meta.url), "utf8");
		const image = source.match(/^image:\s*(\/media\/uploads\/covers\/[^\s]+)$/m)?.[1];
